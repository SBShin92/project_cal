...
//		
//	// 프로젝트 멤버 추가
//	@Insert("INSERT INTO user_project (user_id, project_id) VALUE (#{userID},#{projectId})")
//	int addProjectUser(@Param("userId") int userId,@Param("projectId")int projectId);
//	
//	// 프로젝트 멤버 확인
//	@Select
//		("SELECT COUNT(*) > 0 FROM user_project WHERE user_id = #{userId} AND project_id=#{projectId}")
...
public interface ProjectsDAO {
    ...
    int deleteProjectMember(@Param("projectId") int projectId, @Param("userId") int userId);
