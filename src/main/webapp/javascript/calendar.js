...
const createProjectBars = (date) => {
    ...
    // 사용자가 볼 수 있는 프로젝트만 필터링
    const visibleProjects = dateProjects.filter(project => {
        try {
    ...
    visibleProjects.slice(0, maxVisibleProjects).forEach((project) => {
        // 각 프로젝트에 대한 바 요소 생성
    ...
    if (visibleProjects.length > maxVisibleProjects) {
        ...
        // 초과된 프로젝트 수를 표시
        moreProjectsDiv.textContent = `+${visibleProjects.length - maxVisibleProjects} more`;
        // '더 보기' 요소를 컨테이너에 추가
...
function canViewProject(project) {
    // userCanReadProject가 정의되지 않았을 경우 기본값으로 false를 사용
    return typeof userCanReadProject !== 'undefined' ? userCanReadProject : false;
}
